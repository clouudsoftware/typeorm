...
import { OracleReservedWordError } from "../../error/OracleReservedWordError";
...
export class OracleQueryRunner extends BaseQueryRunner implements QueryRunner {
    ...
    protected createTableSql(table: Table, createForeignKeys?: boolean): Query {
        ...
        if (table.checks.length > 0) {
            const checksSql = table.checks.map(check => {
                ...
                const escapedExpression = check.expression!.replace(/"/g, "");
                return `CONSTRAINT ${checkName} CHECK (${escapedExpression})`;
            }).join(", ");
    ...
    protected buildCreateColumnSql(column: TableColumn) {
        this.checkIfOracleReservedWord(column.name.toUpperCase());
    ...
    private checkIfOracleReservedWord(columnName: string) {
        if(this.driver.reservedWords.includes(columnName)) {
            throw new OracleReservedWordError(columnName);
        }
    }
