...
export class OracleRawSqlResultsToEntityTransformer {
    ...
    protected group(rawResults: any[], alias: Alias): Map<string, any[]> {
        ...
        rawResults.forEach(rawResult => {
            const id = keys.map(key => {
                const keyValue = rawResult[key.toUpperCase()];
    ...
    protected transformColumns(rawResults: any[], alias: Alias, entity: ObjectLiteral, metadata: EntityMetadata): boolean {
        ...
        metadata.columns.forEach(column => {
            ...
            const value = rawResults[0][DriverUtils.buildColumnAlias(this.driver, alias.name.toUpperCase(), column.databaseName.toUpperCase())];
